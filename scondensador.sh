...
DATE_FILE_POS=$(basename $(find "originales/" -name "${DAY}_pos_*_*.csv") | sed -e "s@^.*_pos_\(.*\).csv@\1@g")
DATEP="${DATE_FILE_POS}"
...
function desambiguous(){
    ...
    NEW_LINESA=0
    TOD="$1"
    TOD_TMP="$(echo -e "${TOD}" | sed -e 's@^\(.*\)/\(.*\)/\(.*\)$@\3/\2/\1@g')"
    YES="$(date -d "${TOD_TMP} -1 days" +"%d/%m/%Y")"
    TOD_A=$(echo -e "${TOD}" | sed -e "s@^\(.*\)/\(.*\)/\(.*\)\$@\3\2\1@g")
#    COND_NAME="$(ls -l "${COND_PATH}" | grep -e "condensado_${TOD_A}_.*.csv$" | tail -n 1 | sed -e "s@.*condensado_\(.*\)\$@condensado_\1@g")"
...
#	    echo -e "Y:\n${YES_LINES}"
#            echo -e "T:\n${TOD_LINES}"
#	    echo -e "${FILE_SM_COMP_LINE}"
#	    echo -e "${FILE_SM_COMP_LINES_COU} / ${FILE_SM_COMP_LINES_LEN}"
...
#		    echo -e "${REP} YE_TN lt0 DEL ${DEL_LINES}"
...
#                   echo -e "${REP} YE_TE lt0 del ${DEL_LINES}"
		    LINE_SUS=$(grep -e "^${YES},.*onfirmado,.*,${FILE_SM_COMP_LINE}," "${FILE_SM}")
		    LINE_SUS_HEAD=$(echo -e "${LINE_SUS}" | head -n "${YES_LINES_LEN}")
                    echo -e "${LINE_SUS_HEAD}" >> "${COND_PATH}${RETI_NAME}"
#                   echo -e "ALL:\n${LINE_SUS}"
#                   echo -e "HEA:\n${LINE_SUS_HEAD}"
...
function main(){
    FILE_POS_EXIST=$(ls -l "condensados/" | grep -e "condensado_${DATEP}.csv" ; echo $?)
    if [[ "${FILE_POS_EXIST}" = "1" ]];then
        echo -e "\nSe inicia condensado"
        condensDo $@
        echo -e "\nSe inicia conteo de casos"
        desambiguous $1
    else
	echo -e "No se crea el archivo condensados/condensado_${DATEP}.csv, ya existe"
    fi
}
